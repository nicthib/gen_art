...
save_path = '/gdrive/MyDrive/GANderdash/'
...
def displ_save(img, id):
  img = np.array(img)[:,:,:]
  img = np.transpose(img, (1, 2, 0))
  imageio.imwrite(save_path + str(id) + '.png', np.array(img))
  return None
...
def train(epoch, i, checkpointn):
  loss1 = ascend_txt()
  loss = loss1[0] + loss1[1] + loss1[2]
  optimizer.zero_grad()
  loss.backward()
  optimizer.step()
  if itt % checkpointn == 0 and itt > 0:
    checkin(loss1)
...
def runGAN(lyrics,iters):
  iter = 0
  for line in lyrics:
    if iter > -1:
      itt = 0
      tx = clip.tokenize(line)
      seed_num = randi(0,99999)
      ...
      for i in range(iters):
        train(iter, i, iters-1)
        itt+=1
      with torch.no_grad():
        al = (model(*lats(), 1).cpu()).numpy()
        file_name = 'id{}_seed{}[{}]'.format(iter,seed_num,line)
        print(line)
        displ_save(al[0],file_name)
    iter += 1
